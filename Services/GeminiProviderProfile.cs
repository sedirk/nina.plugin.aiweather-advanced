    ...
    internal static class GeminiProviderProfile
    {
        ...
        public static IReadOnlyList<string> ParseFreeModelOrder(string? serialized)
        {
            ...
            foreach (var model in DefaultFreeModelOrder)
            {
                ...
                {
                    // Insert new defaults before the next known model. Existing custom
                    // ordering remains intact, including a manually moved 3.8 entry.
                    var successors = DefaultFreeModelOrder.SkipWhile(item => item != model).Skip(1);
                    var next = successors.FirstOrDefault(item => configured.Contains(item, StringComparer.OrdinalIgnoreCase));
                    var index = next == null ? -1 : configured.FindIndex(item => string.Equals(item, next, StringComparison.OrdinalIgnoreCase));
                    if (index < 0) configured.Add(model);
                    else configured.Insert(index, model);
                }
            }
