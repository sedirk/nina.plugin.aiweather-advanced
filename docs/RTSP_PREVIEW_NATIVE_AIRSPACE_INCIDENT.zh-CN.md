...
`1.32.0.0` 又修复了一个更晚发生的就绪竞态：播放器可能已经进入 `Playing`，但前 6 秒没有公开尺寸、Vout 或可解码快照；其后同一个播放器却能向 AI 抓帧提供有效图片。旧逻辑在 6 秒窗口结束时永久保留启动盖板，所以识别继续更新而画面仍为空。现在初始窗口只决定是否立即显示，不再决定播放器终身失败：插件会继续对**同一个 MediaPlayer**进行最多约一分钟的有界探测；只要后续尺寸或共享快照成功，就立刻按真实宽高布局并揭开表面。AI 抓帧路径拿到有效共享快照时也会把它作为显示就绪证据。整个恢复过程不重建播放器、不打开第二条 RTSP 会话，停止或替换播放器会取消旧探测。
...
2026-08-30 `1.32.0.0` 延迟表面恢复验证：

- 现场日志证明播放器重启后已进入 `Playing`，但 6 秒内没有 Vout/尺寸；约两分钟后同一个播放器又成功向 AI 提供快照，故障位于显示就绪状态机而非相机连接、模型或第二解码链路；
- 新增有界恢复测试覆盖“第三次同会话探测才成功”、达到最大次数后退出，以及播放器替换时取消三条路径；
- Release 编译与完整冒烟套件通过，预览恢复路径只接收调用方提供的同一播放器探测函数，本身没有创建媒体或 RTSP 连接的能力。
