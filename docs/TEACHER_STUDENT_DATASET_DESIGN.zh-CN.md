...
本文档把“在线视觉模型作为教师、在插件正常工作时同步积累本地数据集、训练并逐步验证本地学生模型”的设想固化为正式工程设计，并同时记录当前实施状态。版本 0.7 已完成阶段 0 / 阶段 1 的最小数据闭环、阶段 2 的人工复核基线、检查器中英文界面、可选的太阳高度门控、按成功检查比例采样和按原图比例降采样，并为 Gemini 增加配额感知熔断与独立在线调用比例；数据导出/冻结、训练管线、ONNX 学生模型和本地优先安全决策仍属于后续阶段。
