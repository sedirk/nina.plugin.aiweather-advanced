    ...
    public static class UiLocalization
    {
        ...
        private static readonly IReadOnlyDictionary<string, Translation> Strings =
            new Dictionary<string, Translation>(StringComparer.Ordinal)
            {
                ...
                ["Options.ClusterReplica"] = new("Replica (read primary status only)", "从节点（仅读取主节点状态）"),
                ...
                ["Options.ClusterReplicaHelp"] = new("Replica mode never opens the camera, calls a model/API, or writes the dataset. It starts Unsafe and can become Safe only after a fresh authenticated primary status.", "从节点模式不会打开相机、不会调用模型/API，也不会写入数据集。连接后初始为不安全，只有收到经过认证的新鲜主节点状态后才可能变为安全。"),
                ["Options.ClusterSecurityHelp"] = new("The token authenticates LAN requests but does not encrypt them. Restrict the port to trusted source IPs with Windows Firewall; do not expose it to the public Internet.", "令牌用于认证局域网请求，但不提供传输加密。请用 Windows 防火墙把端口限制到可信来源 IP，切勿直接暴露到公网。"),
                ...
                ["Options.ProviderNameLocal"] = new("Local (offline heuristic)", "本地（离线启发式算法）"),
                ...
                ["Options.GitHubRetired"] = new("GitHub retired this service on July 30, 2026 — it no longer works for anyone. Analyses fall back to the Local heuristic; switch to Ollama/Custom (local, free), Google Gemini or OpenAI.", "GitHub 已于 2026 年 7 月 30 日停止此服务，现已无法使用。分析会回退到本地启发式算法；请改用 Ollama/自定义服务（本地、免费）、Google Gemini 或 OpenAI。"),
                ...
                ["Options.DatasetHelp"] = new("Only successful online responses become trainable teacher labels. The local heuristic runs in shadow mode and never changes the active safety decision while the teacher succeeds.", "只有成功的在线响应才会成为可训练的教师标签。本地启发式算法以影子模式运行；只要教师模型成功，它就不会改变当前安全判断。"),
                ...
                ["Options.ProviderLocal"] = new("○   Local - offline heuristic analysis using brightness, color, and pattern detection", "○   本地——通过亮度、颜色和图案检测进行离线启发式分析"),
                ...
                ["Runtime.ModelsLocal"] = new("Local analysis — no model selection needed", "本地分析——无需选择模型"),
